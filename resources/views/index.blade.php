                ...
                <div class="container w-70 m-2 py-2" style="background-color: rgb(187, 187, 187)">
                    <ol class="list-group">
                        ...
                        @if (count($todo->todolist))
                            ...
                            @endforeach
                        @else
                            <div class="alert alert-success  w-75 mt-5 align-self-center">No To do list yet !</div>
                        @endif

                    </ol>
                    @if (count($todo->todolist) < 2)
                        <div class="alert alert-success  mt-5 align-self-center">
                            You have <b> {{ count($todo->todolist) }} </b> task in <b>{{ $todo->title }} To do List</b>...
                        </div>
                    @else
                        <div class="alert alert-success  mt-5 align-self-center">
                    ...
                    @endif
                    <hr>
                    <a href="/remove/{{ $todo->id }}"><span class="btn btn-danger mb-2 h5"><i class="fas fa-trash"></i></span></a>
